...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests().antMatchers(unloggedUrls).permitAll().antMatchers("/admin/**").hasAuthority("ADMIN")
                .antMatchers("/district/**").hasAuthority("DISTRICT").antMatchers("/state/**").hasAuthority("STATE")
                .antMatchers("/deo/**").hasAuthority("DEO").anyRequest().authenticated().and().csrf().disable()
                .formLogin().loginPage("/login").authenticationDetailsSource(detailsSource)
